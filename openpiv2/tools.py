...
def display_vector_field(
    ...
    **kw
    ...
    # parse
    x, y, u, v, mask = a[:, 0], a[:, 1], a[:, 2], a[:, 3], a[:, 6]
...
def save(x, y, u, v, speed, vorticity, mask, filename, fmt="%8.4f", delimiter="\t"):
    ...
    """
    if isinstance(u, np.ma.MaskedArray):    #判断u是否有nan元素，有则用0填充
        u = u.filled(0.)
        v = v.filled(0.)

    # build output array
    out = np.vstack([m.flatten() for m in [x, y, u, v, speed, vorticity, mask]])
